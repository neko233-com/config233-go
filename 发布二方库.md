...
1. **创建私有仓库**
   ```bash
   # 在公司 Git 服务器上创建私有仓库
   # 例如：git.company.com/pkg/config233-go
   ```

2. **推送代码**
   ```bash
   git remote add origin git@company.com:pkg/config233-go.git
...
```yaml
variables:
  GOPRIVATE: "git.company.com/pkg/*"
  GONOPROXY: "git.company.com/pkg/*"
